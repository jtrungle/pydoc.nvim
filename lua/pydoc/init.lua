...
function M.open_docs(pkg)
  if not pkg then
	...
	local cmd =
		string.format("%s -c %s %s 2>&1", config.python_cmd, vim.fn.shellescape(script), vim.fn.shellescape(pkg))
...
function M.complete(arglead, _, _)
	local script =
		[[import importlib.metadata as md; print(' '.join(sorted(d.metadata['Name'] for d in md.distributions())))]]
